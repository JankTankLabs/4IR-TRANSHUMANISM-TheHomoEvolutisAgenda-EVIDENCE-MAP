...
assert(/function scrollToCardTop\(id\)\s*\{\s*if \(!id\) return;\s*requestAnimationFrame\(\(\) => scrollToElementTop\(document\.querySelector\(`\[data-id="\$\{id\}"\]`\)\)\);\s*\}/.test(html), 'card top-align scroll should be deferred through requestAnimationFrame');
...
assert(html.includes("openCardById(item.dataset.timelineId, { replaceExpanded: true, interactionId: 'deep-diver' });"), 'timeline card click should route through shared open-card logic');
